"""CP4-G reversible migration gate: unified selection must preserve legacy G."""
...
from tools.g_unified_adapter import GMorningEventAdapter, GProductionRoute, STYLE_ID, UNIT_ID
...
def _route(mode: str = "unified") -> GProductionRoute:
    return GProductionRoute(RegistryLoader().load(_registry(mode)))


def _result(ok: bool = True) -> dict:
    return {"asset": "xauusd", "ok": ok, "style": brief_story.STYLE_G,
            "style_name": "G — บทรอเหตุการณ์", "findings": [],
            "folder": "G-รอเหตุการณ์", "timeframe": "1h"}
...
def test_registry_selects_g_unit_and_preserves_rollback_policy():
    route = GProductionRoute.load()
    ...
    assert entry.letter == "G"
    assert entry.adapter == "fg_brief"
    assert entry.timeframes == ("1h",)


def test_unified_route_calls_legacy_g_pipeline_once_with_exact_arguments(tmp_path):
    raw = _result()
    ...
    runner.assert_called_once_with(asset="xauusd", style=brief_story.STYLE_G,
...
def test_rollback_toggle_bypasses_orchestrator(tmp_path):
    raw = _result()
    with mock.patch.object(brief_pipeline, "run", return_value=raw) as runner, \
            mock.patch("tools.g_unified_adapter.UnifiedStyleOrchestrator.execute") as execute:
...
def test_invalid_or_shadow_route_fails_closed_before_pipeline(tmp_path):
    ...
    invalid = _registry()
    invalid["execution_units"][UNIT_ID]["members"] = ["f_morning_range"]
    with pytest.raises(RegistryError, match="members"):
        GProductionRoute(RegistryLoader().load(invalid)).run_round(
...
def test_adapter_failure_isolated_and_legacy_exception_observable(tmp_path):
    ...
    with mock.patch.object(brief_pipeline, "run", return_value=_result(False)):
        envelope = GMorningEventAdapter(publish_root=tmp_path / "output", cutoff_at=CUTOFF)(
...
def test_run_daily_g_registry_failure_precedes_all_pipeline_side_effects():
    with mock.patch.object(GProductionRoute, "load", side_effect=RegistryError("fixture invalid")), \
