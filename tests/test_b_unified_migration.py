"""CP5-B reversible migration gate: unified selection preserves legacy B."""
...
from tools.b_unified_adapter import BTechnicalAdapter, BProductionRoute, STYLE_ID, UNIT_ID
...
def _route(mode: str = "unified") -> BProductionRoute:
    return BProductionRoute(RegistryLoader().load(_registry(mode)))
...
def test_registry_selects_b_unit_with_rollback_legacy():
    route = BProductionRoute.load()
    ...
    assert route.registry.styles[STYLE_ID].letter == "B"
...
def test_unified_route_calls_legacy_public_builder_once_with_exact_arguments(tmp_path):
    ...
    with mock.patch.object(build_daily_package, "build_public", return_value=raw) as builder:
        result = _route().run_round(
            asset="xauusd", batch_id="batch-b", output_root=tmp_path / "work",
            publish_root=tmp_path / "output", cutoff_at=CUTOFF,
            calendar_feed_enabled=False,
        )
    assert result is raw
    builder.assert_called_once_with(
        "xauusd", batch_id="batch-b", output_root=tmp_path / "work",
...
def test_rollback_toggle_bypasses_orchestrator(tmp_path):
    raw = _result()
    with mock.patch.object(build_daily_package, "build_public", return_value=raw) as builder, \
            mock.patch("tools.b_unified_adapter.UnifiedStyleOrchestrator.execute") as execute:
...
def test_invalid_or_shadow_mode_fails_closed_before_legacy_side_effect(tmp_path):
    ...
    with pytest.raises(RegistryError, match="members"):
        BProductionRoute(RegistryLoader().load(invalid))
...
def test_adapter_failure_isolated_and_legacy_result_is_observable(tmp_path):
    ...
    adapter = BTechnicalAdapter(batch_id="batch-b", output_root=tmp_path / "work",
