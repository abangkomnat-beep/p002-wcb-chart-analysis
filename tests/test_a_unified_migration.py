"""CP5-A reversible migration gate: unified selection preserves legacy A/B/C."""
...
from tools.a_unified_adapter import APublicAdapter, AProductionRoute, STYLE_ID, UNIT_ID
...
def _route(mode: str = "unified") -> AProductionRoute:
    return AProductionRoute(RegistryLoader().load(_registry(mode)))
...
def test_registry_selects_a_unit_and_does_not_register_b_or_c():
    route = AProductionRoute.load()
    ...
    assert {entry.letter for entry in route.registry.styles.values()} == {"A", "D", "E", "F", "G", "H", "I", "J"}
...
def test_unified_route_calls_legacy_public_builder_once_with_exact_arguments(tmp_path):
    ...
    with mock.patch.object(build_daily_package, "build_public", return_value=raw) as builder:
        result = _route().run_round(
            asset="xauusd", batch_id="batch-a", output_root=tmp_path / "work",
            publish_root=tmp_path / "output", cutoff_at=CUTOFF,
            calendar_feed_enabled=False,
        )
    assert result is raw
    builder.assert_called_once_with(
        "xauusd", batch_id="batch-a", output_root=tmp_path / "work",
...
def test_rollback_toggle_bypasses_orchestrator(tmp_path):
    raw = _result()
    with mock.patch.object(build_daily_package, "build_public", return_value=raw) as builder, \
            mock.patch("tools.a_unified_adapter.UnifiedStyleOrchestrator.execute") as execute:
...
def test_invalid_or_shadow_mode_fails_closed_before_legacy_side_effect(tmp_path):
    ...
    with pytest.raises(RegistryError, match="members"):
        AProductionRoute(RegistryLoader().load(invalid))
...
def test_adapter_failure_isolated_and_legacy_result_is_observable(tmp_path):
    ...
    adapter = APublicAdapter(batch_id="batch-a", output_root=tmp_path / "work",
