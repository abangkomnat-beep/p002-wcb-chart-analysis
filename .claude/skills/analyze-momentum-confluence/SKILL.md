...
สำหรับ WCB Daily Output Contract v3.2 ให้แยกทิศแนวโน้มออกจากคุณภาพตำแหน่งเข้า เช่น แนวโน้มลงแต่ `entry_quality=poor` เมื่อราคายืดลงมากแล้ว จำกัด indicator ที่เสนอเผยแพร่ 2–3 ตัวที่หน้าที่ไม่ซ้ำ และรวมทั้งบทไม่เกิน 5 ตัว

⚠️ **รุ่นปัจจุบันของ P002** — ระบบคำนวณจริง 5 ตัวเท่านั้น: `sma20` `sma50` `sma200` `rsi14` `atr14` (`tools/indicators.py`)
MACD และ Bollinger **ยังไม่มีในสายท่อ** ⇒ รายงานเป็น `unavailable` · ข้อมูลรายวัน (D1) เท่านั้น ⇒ `entry_quality` ที่อ้างจังหวะระหว่างวันต้องเป็น `unavailable`
