...
แหล่งข้อมูลราคาหลักคือ **WCB series API** (`api/md?kind=series`) เรียกผ่าน HTTP ธรรมดา
**ไม่ต้องใช้รหัสเข้าถึง ไม่ต้องติดตั้งโปรแกรมเพิ่ม และรันได้ทุกระบบปฏิบัติการ**
ครอบคลุมหัวข้อ `xauusd` · `eurusd` · `nvda`

**ข้อยกเว้นข้อเดียว — `btcusd` ยังต้องใช้ MetaTrader 5**

ปลายทางไม่มีแท่งวันเสาร์อาทิตย์ของคริปโท (วัดจริง 2026-08-05 ทั้งกรอบวันและกรอบสี่ชั่วโมง)
ราคาที่วิ่งช่วงสุดสัปดาห์จะถูกยุบเป็นช่องว่างของแท่งวันจันทร์ ⇒ ฐานคำนวณแนวรับแนวต้าน
ของบทเช้าวันจันทร์จะผิดจริง ไม่ใช่แค่หยาบ จึงยังไม่ย้าย **ถ้าไม่ทำหัวข้อคริปโท ข้ามส่วนนี้ได้ทั้งหมด**

ถ้าจะรัน `btcusd` ต้องมี:
...
3. สัญลักษณ์ `BTCUSD` อยู่ใน Market Watch

แหล่งข้อมูลไม่พร้อมหรือข้อมูลค้างเก่า = **สายท่อหยุดสินทรัพย์นั้นและไม่เขียนบทความ**
ไม่มีการสลับไปแหล่งอื่นเองเงียบ ๆ

ชั้นตรวจข้อมูล ชั้นภาษา และชุดเทสไม่ต้องต่อเน็ตและไม่ต้องใช้ MT5 (เทสใช้ปลายทางปลอม)

## ติดตั้งและรัน

```bash
pip install -r requirements.txt          # matplotlib/pillow สำหรับกราฟ · MetaTrader5 เฉพาะถ้าจะรัน btcusd
...
# สามหัวข้อที่ไม่ต้องพึ่ง MT5 — ดึงจาก WCB series API เป็นค่าตั้งต้น
python -m tools.build_daily_package --asset xauusd --asset eurusd --asset nvda \
    --batch-id 2026-08-05T09-00Z-daily-market

# ครบสี่หัวข้อ — btcusd จะไปดึงจาก MT5 เอง ต้องเปิด terminal ค้างไว้
...
| ธง | ความหมาย |
|---|---|
| _(ไม่ระบุ)_ | ตามค่าตั้งต้นของหัวข้อนั้น — WCB series API ทุกหัวข้อ ยกเว้น `btcusd` ที่เป็น MT5 |
| `--source wcb` | บังคับใช้ WCB series API |
| `--source mt5` | บังคับใช้ MT5 — เก็บไว้ทานสอบราคาสองแหล่ง (ต้องเปิด terminal) |
...
- **หนึ่งรอบใช้แหล่งเดียว** — ยังไม่มี cross-provider verification อัตโนมัติในสายท่อ (fatal rule ข้อ 6) ตามการตัดสินของผู้ใช้ 2026-08-03 ตัวเลขในบทความจึงยืนยันได้เท่าที่แหล่งเดียวรายงาน
  ตอนนี้มีสองแหล่งให้ทานสอบด้วยมือแล้ว (`--source wcb` เทียบ `--source mt5`) ทำครั้งแรก 2026-08-05
  กับทองคำ 130 แท่ง: ชุดวันที่ตรงกันทั้งหมด · ส่วนต่างราคาปิดมัธยฐาน 0.015% · Pivot ต่างไม่เกิน 0.28 ดอลลาร์
...
- 🔒 **ระบบนี้เดินในโหมดใช้ภายในเท่านั้น (by design)** — ด่านสิทธิ์คง `approved-internal-only` ทุกครั้ง ด้วยเหตุผลคนละข้อของสองแหล่ง:
  - `mt5_raw_trading` — อ่าน Client Agreement ของ Raw Trading Ltd ครบแล้วเมื่อ 2026-08-04: ลิขสิทธิ์และสิทธิ์ฐานข้อมูลใน Quotes เป็นของโบรก (ข้อ 28.7) และให้ใช้เพื่อ personal use เท่านั้น (ข้อ 10.19) ⇒ **ไม่มีสิทธิ์เผยแพร่ ชัดเจนแล้ว**
...
- **`btcusd` ต้องรันบน Windows ที่มี terminal MT5 ล็อกอินค้างไว้** — อีกสามหัวข้อรันผ่าน HTTP ล้วน ดูหัวข้อข้อกำหนดก่อนรัน
