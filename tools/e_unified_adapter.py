"""Thin reversible production route for the Style E family.

BTCUSD is delivered by the dual-timeframe E+ daily layer.  Existing Style E
assets still delegate to the legacy indicator pipeline unchanged; ``legacy``
mode remains the immediate rollback path for the complete execution unit.
"""
...
from . import (chart_indicator_pipeline, chart_indicator_writer,
               style_e_plus_daily, zone_memory)
...
class EIndicatorAdapter:
    ...
    def __call__(self, context, asset, _plan, _output, _state) -> ResultEnvelope:
        try:
            runner = (style_e_plus_daily.run if asset == style_e_plus_daily.ASSET
                      else chart_indicator_pipeline.run)
            raw = runner(asset=asset, publish_root=self.publish_root,
        ...
        if not isinstance(raw, Mapping):
            return ResultEnvelope.fail(
                ...
                findings=[Finding("runner_result", "fatal", "Style E runner returned a non-mapping result")],
            )
...
class EProductionRoute:
    ...
    def load(cls, path: Path | None = None) -> "EProductionRoute":
        ...
        required_assets = {style_e_plus_daily.ASSET, "xauusd", "usdjpy"}
        if set(entry.assets) != required_assets:
            raise RegistryError(
                f"{UNIT_ID} assets must be {sorted(required_assets)!r}, got {list(entry.assets)!r}")
        btc_variant = (entry.metadata.get("asset_variants") or {}).get(
            style_e_plus_daily.ASSET, {})
        if (btc_variant.get("style") != "E+"
                or btc_variant.get("folder") != style_e_plus_daily.FOLDER
                or tuple(btc_variant.get("timeframes") or ()) != ("1h", "15min")):
            raise RegistryError(f"{UNIT_ID} BTCUSD E+ variant metadata does not match delivery")
        return cls(registry)
    ...
    def run_round(self, *, asset: str, publish_root: Path, cutoff_at: str,
        ...
        requests = [
            {"provider": "WCB", "timeframe": "1h",
             "request_role": "indicator_series", "required_by": (STYLE_LETTER,)}]
        if asset == style_e_plus_daily.ASSET:
            requests.append(
                {"provider": "WCB", "timeframe": "15min",
                 "request_role": "execution_series", "required_by": (STYLE_LETTER,)})
