"""Thin reversible production route for Style B (technical WCB article).

Style B shares the legacy ``build_public`` implementation with A/C.  This
adapter adds only the unified control-plane seam; the legacy writer, output
layout, and publication behavior remain unchanged.
"""
...
UNIT_ID = "B_PUBLIC"
STYLE_ID = "b_technical"
STYLE_LETTER = "B"


class BTechnicalAdapter:
    """Invoke the existing public builder exactly once per eligible asset."""
...
class BProductionRoute:
    """Validated B route; ``legacy`` is the immediate rollback path."""

    registry: ArticleStylesRegistry

    @classmethod
    def load(cls, path: Path | None = None) -> "BProductionRoute":
        ...
        try:
            ...
            raise RegistryError(f"missing Style B registration: {exc.args[0]}") from exc
        ...
        if entry.letter != STYLE_LETTER or entry.adapter != "abc_public":
            raise RegistryError(f"{UNIT_ID} must map to Style B with abc_public adapter")
    ...
    def run_round(self, *, asset: str, batch_id: str, output_root: Path,
        ...
        b_registry = ArticleStylesRegistry(
        ...
        result = UnifiedStyleOrchestrator({UNIT_ID: BTechnicalAdapter(
        ...
        )}).execute(context, b_registry, {UNIT_ID: plan})
        if len(result) != 1:
...
__all__ = ["BTechnicalAdapter", "BProductionRoute", "UNIT_ID", "STYLE_ID"]
