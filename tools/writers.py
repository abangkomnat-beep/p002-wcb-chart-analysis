...
def plan_for_public(trade_branch: dict | None) -> dict | None:
    ...
    3. ด่านความเสี่ยงต้องไม่พบข้อ `blocking` — ข้อระดับนี้แปลว่าตัวเลขในแผน**ผิด**
       (อ้างระดับที่ไม่ได้อนุมัติ · ไม่มีที่มา · ไม่มีทิศแต่ยังเสนอจุดเข้า)

    **ทำไมข้อ `required` ถึงไม่กั้น** — ข้อระดับนั้นวัดกับเกณฑ์ใน `risk_auditor`
    ที่ตัวโมดูลเองประกาศว่ายังไม่ผ่านการวัดกับข้อมูลจริง (รอระยะ 4 และผู้ใช้ล็อกค่า)
    การเอาเกณฑ์ที่ยังไม่ล็อกมาห้ามเผยแพร่ = ตัดสินด้วยเลขที่เรายังไม่เชื่อเอง
    ส่วนตัวเลขที่ได้ยังถูกต้องตามที่วัด — อัตราส่วนต่ำก็เขียนตรง ๆ ว่าต่ำ (ดู `_structure_plan`)
    """
    ...
    if any(item["severity"] == risk_auditor.BLOCKING for item in audit.get("findings") or []):
        return None
    return plan
