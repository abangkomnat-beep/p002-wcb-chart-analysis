"""Thin reversible production route for Style C (event reaction article).

Style C shares the legacy ``build_public`` implementation with A/B.  The
unified route adds only a validated control-plane seam; legacy behavior,
evidence construction, output layout, and publication remain unchanged.
"""
...
class CEventAdapter:
    """Invoke the existing public A/B/C builder exactly once per asset."""
...
class CProductionRoute:
    """Validated C route; ``legacy`` is the immediate rollback path."""

    registry: ArticleStylesRegistry

    @classmethod
    def load(cls, path: Path | None = None) -> "CProductionRoute":
        ...
        try:
            ...
            raise RegistryError(f"missing Style C registration: {exc.args[0]}") from exc
        ...
        if entry.letter != STYLE_LETTER or entry.adapter != "abc_public":
            raise RegistryError(f"{UNIT_ID} must map to Style C with abc_public adapter")
    ...
    def run_round(self, *, asset: str, batch_id: str, output_root: Path,
        ...
        c_registry = ArticleStylesRegistry(
        ...
        result = UnifiedStyleOrchestrator({UNIT_ID: CEventAdapter(
        ...
        )}).execute(context, c_registry, {UNIT_ID: plan})
        if len(result) != 1:
...
__all__ = ["CEventAdapter", "CProductionRoute", "UNIT_ID", "STYLE_ID"]
