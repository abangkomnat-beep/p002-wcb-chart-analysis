"""Thin reversible production route for Style G (morning event brief).

The unified seam delegates the existing G pipeline unchanged.  ``legacy`` is
kept as a direct rollback path and invalid/shadow modes fail closed before any
...
from . import brief_pipeline, brief_story, zone_memory
...
UNIT_ID = "G_BRIEF"
STYLE_ID = "g_morning_event"
STYLE_LETTER = "G"


class GMorningEventAdapter:
    """Invoke the existing G pipeline once with its production arguments."""
    ...
    def __call__(self, context, asset, _plan, _output, _state) -> ResultEnvelope:
        try:
            raw = brief_pipeline.run(
                asset=asset, style=brief_story.STYLE_G,
...
class GProductionRoute:
    """Validated G route; legacy mode is the immediate rollback path."""

    registry: ArticleStylesRegistry

    @classmethod
    def load(cls, path: Path | None = None) -> "GProductionRoute":
        ...
        try:
            ...
            raise RegistryError(f"missing Style G registration: {exc.args[0]}") from exc
        ...
        if entry.letter != STYLE_LETTER or entry.adapter != "fg_brief":
            raise RegistryError(f"{UNIT_ID} must map to Style G with fg_brief adapter")
    ...
    def run_round(self, *, asset: str, publish_root: Path, cutoff_at: str,
        ...
        if self.migration_mode == "legacy":
            return brief_pipeline.run(asset=asset, style=brief_story.STYLE_G,
        ...
        g_registry = ArticleStylesRegistry(
        ...
        result = UnifiedStyleOrchestrator({UNIT_ID: GMorningEventAdapter(
            publish_root=Path(publish_root), cutoff_at=cutoff_at,
            runner_kwargs=kwargs,
        )}).execute(context, g_registry, {UNIT_ID: plan})
        if len(result) != 1:
...
__all__ = ["GMorningEventAdapter", "GProductionRoute", "UNIT_ID", "STYLE_ID"]
