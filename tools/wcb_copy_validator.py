...
# ทะเบียนชื่อหัวข้อที่อนุญาต — **ไวต์ลิสต์ ไม่ใช่คำนำหน้าแบบหลวม ๆ**
#
# 🔄 **เปลี่ยนวิธีจับเมื่อ 2026-08-11 ตามคำสั่งผู้ใช้** — ของเดิมจับด้วยคำนำหน้า
# (`## เทคนิค` / `## ปัจจัย` / `## กลยุทธ`) ซึ่งบังคับกลาย ๆ ให้ทั้งสามสไตล์ตั้งชื่อ
# หัวข้อขึ้นต้นเหมือนกัน ⇒ ชื่อในใบตัวอย่างของผู้ใช้ (`01-CC/Input/ภาษาการเขียน/`)
# สองในสามใช้ไม่ได้เลย · ผู้ใช้สั่งให้ยึดใบตัวอย่างเป็นหลัก ด่านจึงต้องขยับตาม
#
# ⚠️ **ไม่ใช่การผ่อนด่าน** — ยังบังคับครบสามเรื่องเหมือนเดิม (เทคนิค · ข่าว · กลยุทธ์)
# และยังเป็นไวต์ลิสต์ปิดตาย: ชื่อที่ไม่อยู่ในทะเบียนนี้ยังตกทุกตัว รวมทั้งชื่อที่พิมพ์ผิด
# สิ่งที่เปลี่ยนคือทะเบียนถูกผูกกับ **ใบตัวอย่างจริง** แทนคำนำหน้าที่เราคิดขึ้นเอง
# ⇒ เพิ่มสไตล์ใหม่หรือแก้ชื่อหัวข้อใน `wcb_writers` เมื่อไหร่ ต้องมาเติมที่นี่ด้วยเสมอ
#
# 🪤 **ข้อที่ยังไม่ได้คำตอบ:** สเปกของหัวหน้าเคยระบุว่าเว็บใช้ชื่อหัวข้อสามชื่อนี้
# จัดโครงหน้า แต่หลักฐานที่วัดเองชี้ไปคนละทาง (เทมเพลตของเว็บเองใช้
# `<h2>ทำไมยูโรถึงเด้งขึ้นรอบนี้`) — ตั้งเป็นคำถามไว้ในใบแจ้ง E23 แล้ว
# ถ้าเว็บยืนยันว่าผูกกับชื่อจริง ทะเบียนนี้คือจุดเดียวที่ต้องแก้
_H2 = r"(?m)^##\s*(?:\d+\.\s*)?"
REQUIRED_HEADINGS = (
    ("หัวข้อเทคนิค", re.compile(
        _H2 + r"(?:ภาพรวมทางเทคนิค|เจาะโครงสร้างราคา|แนวรับ-แนวต้าน)")),
    ("หัวข้อปัจจัยข่าว", re.compile(
        _H2 + r"(?:ปัจจัยข่าว|ปฏิทินข่าว)")),
    ("หัวข้อกลยุทธ์", re.compile(
        _H2 + r"(?:วางแผนและกลยุทธ|สรุปกลยุทธ|ฉากทัศน์)")),
)
...
STRUCTURAL = (
    re.compile(r"\[\[chart:[^\]]*\]\]"),
    # เลขลำดับหัวข้อ ("## 2. ด่านราคาสำคัญ…") — เป็นเลขนับหัวข้อในเอกสาร ไม่ใช่ค่าตลาด
    # จึงไม่มีวันมีต้นทางใน snapshot · เพิ่ม 08-11 พร้อมชื่อหัวข้อชุดใหม่ที่ใส่เลขลำดับ
    # **จับเฉพาะที่ต้นบรรทัดหัวข้อ** ไม่ใช่ทุก "เลข." ในบท — ไม่งั้นจะกลายเป็นช่องโหว่
    # ให้เลขจริงที่ตามด้วยจุดหลุดด่านไปทั้งหมด (`voice_rules` ใช้กติกาเดียวกัน)
    re.compile(r"^\s*#{2,3}\s*\d+\."),
    # เลขลำดับลิสต์ ("  1. **4,366.37** ดอลลาร์…") — เครื่องหมายโครงสร้าง markdown
    # เหมือนขีดของ bullet ไม่ใช่ค่าตลาด (เพิ่ม 08-11 พร้อมแนวรับ/แนวต้านแบบลำดับของ A)
    # **จับเฉพาะต้นบรรทัด + ตามด้วยช่องว่าง** ด้วยเหตุผลเดียวกับเลขหัวข้อข้างบน —
    # เลขจริงกลางบรรทัดที่ตามด้วยจุด (เช่น "4.36") ยังเข้าด่านครบทุกตัว
    re.compile(r"^\s{0,8}\d{1,2}\.\s"),
...
def collect_evidence(payload) -> set[float]:
    ...
    walk(payload)
    # 🔄 08-11: เลิกขึ้นทะเบียนปี พ.ศ. คู่กับ ค.ศ. — บทกลับมาเขียน ค.ศ. ซึ่งเป็นระบบ
    # เดียวกับที่ก้อน snapshot เก็บอยู่แล้ว ⇒ ปีในบทชี้กลับก้อนได้ตรง ๆ ไม่ต้องแปลง
    # **การถอดออกทำให้ด่านเข้มขึ้น ไม่ใช่หลวมลง** — เดิม 2569 ผ่านได้เพราะเราเปิดช่องให้
    # ตอนนี้ไม่มีช่องนั้นแล้ว และ `consistency_gate` กฎ 2 ตีตก พ.ศ. ซ้ำอีกชั้น
    return numbers
