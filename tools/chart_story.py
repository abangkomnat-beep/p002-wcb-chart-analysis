...
# 🆕 **เพดานเปอร์เซ็นต์ทับอีกชั้น — หัวหน้าเคาะข้อ 1ก เมื่อ 2026-08-11**
# (ตัวเลขเพดานเขาให้ทีมเราตั้งเอง — เลือก 10% ด้วยเหตุผลข้างล่าง)
#
# **ปัญหาของเกณฑ์ ATR ล้วน:** มันยืดตามความผันผวน ⇒ **หลวมที่สุดในวันที่ควรเข้มที่สุด**
# วัดจริงจากทองใบ 10 ส.ค. (ราคา 4,342.63):
#
#     ช่วง ATR 1.07% (ต้นเดือน) → 10×ATR ≈ ±465 ดอลลาร์ ≈ ±10.7%
#     ช่วง ATR 2.17% (ผันผวน)   → 10×ATR ≈ ±942 ดอลลาร์ ≈ ±21.7%
#
# เกณฑ์ตัวเดียวกันแต่ความหมายต่างกันเท่าตัวภายในเดือนเดียว — และทำให้โซนที่ห่าง
# +21.8% "เกือบผ่าน" ทั้งที่หัวหน้าเคยตีกลับใบที่โซนห่าง +20.6%
#
# **ทำไม 10%:** เป็นเพดานที่ตีตกทั้งสองเคสข้างบนอย่างชัดเจน และในวันตลาดนิ่ง
# ATR ยังเป็นตัวที่เข้มกว่า (10×ATR ≈ 10.7% > 10%) ⇒ เพดานไม่ไปแทนที่ ATR
# แต่มาคุมเฉพาะหางที่ ATR คุมไม่อยู่ · **ทั้งสองเกณฑ์ต้องผ่านพร้อมกัน (AND)**
ENTRY_MAX_DISTANCE_PERCENT = 0.10
...
def build_story(rows: list[dict], *, asset: str,
    ...
    `publish_date` (ตัวเลือก): **วันที่เผยแพร่** ที่พาดหัวจะพิมพ์ — คนละเรื่องกับ
    `current["date"]` ซึ่งเป็นวันของ**แท่งที่ปิดแล้ว**ที่ทุกตัวเลขในบทมาจาก
    (มติผู้ใช้ 2026-08-14: พาดหัวทุกสไตล์ลงวันที่วันนี้ให้ตรงกัน เพราะคำว่า "วันนี้"
    ในพาดหัวต้องเป็นวันนี้จริง และเป็นคำที่คนไทยใช้ค้น — ฟีดแบ็กหัวหน้า S-1)
    · บทวิเคราะห์รายวันที่ออกตอนเช้าย่อมอ้างราคาปิดของเมื่อวานเป็นปกติ ⇒ **เนื้อบท
    ต้องระบุวันของแท่งฐานให้ชัด** ไม่งั้นคนอ่านเข้าใจว่าเป็นราคาปิดของวันนี้
    ⚠️ ไม่ส่งมา = ถอยไปใช้ `current["date"]` เหมือนเดิม (พฤติกรรมก่อน 08-14) —
    ทำให้เทสที่เรียกตรงไม่ผูกกับนาฬิกาเครื่อง · สายผลิตส่งเสมอ และมีเทสคุมว่าส่งจริง
    ...
    story = {
        ...
        "calendar": calendar,
        # ไม่ส่งมา = วันเผยแพร่เท่ากับวันแท่งฐาน (พฤติกรรมก่อน 08-14)
        "publish_date": publish_date or current["date"],
    }
    if memory_meta is not None:
...
def within_daily_entry_range(current: float, level: float, atr: float) -> bool:
    ...
    บังคับใช้ข้ามสไตล์ตามที่หัวหน้าสั่ง 2026-08-07 — สไตล์ E เคยหลุดกฎนี้เพราะไม่มี
    ตัวกรองระยะห่างเลย (Golden Zone ห่างราคา 14.8–20.6% ก็ยังถูกเสนอเป็นแผนหลัก)

    🆕 **สองเกณฑ์ต้องผ่านพร้อมกัน (หัวหน้าเคาะ 1ก · 08-11):**

    1. `ENTRY_MAX_DISTANCE_ATR` — ยืดตามความผันผวน คุมวันตลาดนิ่งได้เข้มกว่า %
    2. `ENTRY_MAX_DISTANCE_PERCENT` — เพดานตายตัว คุมวันตลาดผันผวนที่ ATR หลวมเกิน

    ตัวที่**เข้มกว่า**เป็นตัวคุมเสมอ (ใช้ `min`) — เจตนาคือไม่ให้มีวันไหนที่เกณฑ์หลวม
    กว่าที่ควร ไม่ใช่ให้ผ่านง่ายขึ้น · ⛔ **ห้ามเปลี่ยนเป็น `or`** นั่นคือการผ่อนด่าน
    """
    ceiling = min(ENTRY_MAX_DISTANCE_ATR * atr,
                  ENTRY_MAX_DISTANCE_PERCENT * abs(current))
    return abs(current - level) <= ceiling
