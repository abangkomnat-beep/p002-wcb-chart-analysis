"""Thin reversible production route for Style D (chart story).

The unified adapter delegates the complete legacy D pipeline unchanged; the
route only selects the control plane and validates the D registry seam before
any data or filesystem side effect.
"""
...
from . import chart_story_pipeline, chart_story_writer, zone_memory
...
UNIT_ID = "D_CHART_STORY"
STYLE_ID = "d_chart_story"
STYLE_LETTER = "D"


class DChartStoryAdapter:
    """Invoke the existing D pipeline once with production arguments."""
    ...
    def __call__(self, context, asset, _plan, _output, _state) -> ResultEnvelope:
        ...
        if not isinstance(raw, Mapping):
            return ResultEnvelope.fail(
                ...
                findings=[Finding("legacy_result", "fatal", "chart_story_pipeline.run returned a non-mapping result")],
            )
...
class DProductionRoute:
    """Validated D route; legacy mode is the immediate rollback path."""

    registry: ArticleStylesRegistry

    @classmethod
    def load(cls, path: Path | None = None) -> "DProductionRoute":
        ...
        try:
            ...
            raise RegistryError(f"missing Style D registration: {exc.args[0]}") from exc
        if tuple(unit.members) != (STYLE_ID,):
            raise RegistryError(f"{UNIT_ID} members must be {(STYLE_ID,)!r}, got {tuple(unit.members)!r}")
        if entry.letter != STYLE_LETTER or entry.adapter != "d_chart_story":
            raise RegistryError(f"{UNIT_ID} must map to Style D with d_chart_story adapter")
    ...
    def run_round(self, *, asset: str, publish_root: Path, cutoff_at: str,
        ...
        if self.migration_mode == "legacy":
            return chart_story_pipeline.run(asset=asset, publish_root=publish_root,
        ...
        plan = SourcePlanner().plan(
            ...
            requests=(
                {"provider": "WCB", "timeframe": "1d", "request_role": "context_series",
        ...
        context = RunContext(
            ...
            state_root=Path(zone_memory.STATE_DIR),
            mode="unified", no_publish=False,
        )
        # The production registry contains other units; this change set must
        # execute only D so CP3 cannot accidentally migrate H/I/J again.
        d_registry = ArticleStylesRegistry(
        ...
        result = UnifiedStyleOrchestrator({UNIT_ID: DChartStoryAdapter(
            publish_root=Path(publish_root), cutoff_at=cutoff_at,
            runner_kwargs=_runner_kwargs,
        )}).execute(context, d_registry, {UNIT_ID: plan})
        if len(result) != 1:
...
__all__ = ["DChartStoryAdapter", "DProductionRoute", "UNIT_ID", "STYLE_ID"]
