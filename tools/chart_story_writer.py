...
def render_article(story: dict) -> str:
    ...
    asset_name = f"ราคา{profile['seo_name']} ({story['symbol']})"
    if down:
        opening_first = (
    ...
    if rows:
        gates = "สองจุด" if len(rows) > 1 else "จุดเดียว"
        lines += [f"ภาพรวมราคา{profile['seo_name']}วันนี้สรุปเหลือเพียง "
                  f"**\"{gates}ตัดสินใจสำคัญ (Key Trigger Levels)\"** "
