...
def render_article(story: dict) -> str:
    ...
    # "อธิบายเป็นลำดับขั้นตอน พร้อมทำเป็น bullet ให้เข้าใจง่ายขึ้น")** — เนื้อความชุดเดิม
    # จัดรูปใหม่ผ่าน `wcb_writers.listing`/`nested_listing` ซึ่งเคารพสวิตช์ bullet
    # ของเว็บเหมือนสไตล์อื่น · ศัพท์สองคำเปลี่ยนตามคำสั่งเดียวกัน:
    ...
    if entries:
        entry_lines += [
            ...
            "พร้อมจุด Stoploss ชัดเจนทุกจุดครับ",
            "",
        ]
        entry_groups = []
        for entry in entries:
            ...
            entry_groups.append((
                ...
                [f"โซนนี้ถูกใช้เป็นแนวอ้างอิงมาแล้ว {entry['touches']} ครั้ง — "
                 ...
                 f"จุด Stoploss: ราคาปิดวันต่ำกว่า "
                 f"{money(entry['invalidation'])} ดอลลาร์"]))
        entry_lines += wcb_writers.nested_listing(entry_groups)
        if entry_lines[-1] != "":     # โหมดร้อยแก้วไม่ปิดบรรทัดว่างให้ — เติมเองก่อนบล็อกถัดไป
            entry_lines.append("")
        ...
        if excluded:
            ...
            entry_lines += [" และ ".join(excluded_parts)
                            + " อยู่ห่างจากราคาปัจจุบันเกินเกณฑ์แผนรายวันของระบบ "
                            "จึงไม่จัดเป็นจุดเข้าในบทนี้ — เป็นระดับเชิงโครงสร้างกรอบหลายเดือนเท่านั้น",
                            ""]
        entry_lines += ["**Execution Plan — ลำดับขั้นตอนก่อนเข้าเทรด:**", ""]
        entry_lines += wcb_writers.listing("", [
            "**ขั้นที่ 1 —** รอราคาย่อกลับเข้าโซนก่อน อย่ารีบสวนมีดขณะราคากำลังร่วง "
            "และอย่าไล่ราคากลางช่องว่างครับ",
            "**ขั้นที่ 2 —** สลับไป Timeframe ย่อย (1H/15M) เพื่อหาการยืนยันตัวตนของ "
            "Smart Money เช่นการเกิด CHoCH (Change of Character) หรือ Stop Hunt "
            "ที่ราคาจิ้มกวาดสภาพคล่องใต้โซนแล้วดีดกลับอย่างรวดเร็ว",
            "**ขั้นที่ 3 —** ได้สัญญาณยืนยันแล้วจึงค่อยพิจารณาเข้า — "
        ...
        if story["current"]["close"] > entries[0]["zone_high"]:
            entry_lines += ["",
                            ...
                            "การเข้ากลางอากาศคือการยอมเสียทั้งอัตราส่วนความเสี่ยงต่อผลตอบแทน"
                            "และตำแหน่งจุด Stoplossที่ดี — การรอให้ราคาลงมาหาเราคือหัวใจของแนวคิดนี้ครับ"]
    elif story["zones"]:
