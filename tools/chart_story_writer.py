...
def _excerpt_clauses(story: dict) -> list[str]:
    ...
    clauses += ["อ่านโครงสร้างกราฟรายวันพร้อมจุดเข้าและจุด Stoploss ทั้งสองฝั่ง",
...
def render_article(story: dict) -> str:
    ...
    # เปลี่ยน**ทุกที่ในสไตล์ D** ไม่ใช่เฉพาะขั้นที่ 4 ที่ผู้ใช้ยกมา — เป็นชื่อเรียก
    # ของเรื่องเดียวกันทั้งบท ถ้าเปลี่ยนจุดเดียวบทจะมีสองชื่อสำหรับของอย่างเดียวกัน
    # ⚠️ สไตล์ F/G เรียกของสิ่งนี้ว่า "จุดตัดขาดทุน (Stop Loss)" — คนละสะกดกัน
    # ยังไม่รวมให้เหมือนกันเพราะผู้ใช้สั่งเฉพาะสไตล์ D (ค้างไว้ให้ผู้ใช้ตัดสิน)
    #
    ...
    if up:
        bullish_items = [
            f"**เงื่อนไข:** กุญแจอยู่ที่{up['condition']} ({money(up['trigger'])} ดอลลาร์) "
            "— การปิดเหนือระดับนี้ได้จริงจะมีน้ำหนักเป็น Break of Structure (BOS) ฝั่งขึ้น"]
        if up["targets"]:
            targets = " และ ".join(money(value) for value in up["targets"])
            bullish_items.append(f"**เป้าถัดไป:** เปิดทางเข้าหา Supply ที่ {targets} "
                                 "ดอลลาร์ตามลำดับ")
        bullish_items.append(
            f"**จุด Stoploss:** เมื่อ{up['invalidation']} "
        ...
        # เล่าเป็นลำดับขั้นตอนเพราะมันคือกติกาที่ต้องทำ "ตามลำดับ" จริง ข้ามขั้นไม่ได้
        lines += ["**ขั้นตอนเข้าเทรดฝั่งขึ้น (Breakout-Continuation):** "
                  "สำหรับคนที่อยากตามแรงดีด", ""]
        lines += wcb_writers.listing("", [
            f"**ขั้นที่ 1 —** รอให้ราคาปิดวันเหนือ {money(up['trigger'])} ดอลลาร์ให้จบก่อน "
            "อย่าไล่ราคาตอนกำลังทะลุ",
            f"**ขั้นที่ 2 —** รอจังหวะราคาย่อกลับมาทดสอบแนวที่เพิ่งทะลุในช่วง "
            f"{money(up['entry_low'])}–{money(up['entry_high'])} ดอลลาร์ "
            "(Retest — แนวต้านเดิมพลิกเป็นแนวรับ)",
            "**ขั้นที่ 3 —** หาสัญญาณแรงซื้อยืนยันใน Timeframe ย่อยก่อน แล้วจึงพิจารณาเข้า",
            f"**ขั้นที่ 4 —** จุด Stoploss: ราคาปิดวันกลับต่ำกว่า "
            f"{money(up['entry_invalidation'])} ดอลลาร์ — ถึงตรงนั้นการทะลุถือว่าล้มเหลว "
            "ไม่ถัวไม่รอครับ",
        ]) + [""]
    if down_scenario:
        bearish_items = [
            f"**เงื่อนไข:** สัญญาณอันตรายคือ{down_scenario['condition']} "
        ...
        if down_scenario["targets"]:
            ...
            bearish_items.append(f"**เป้าถัดไปของฝั่งขาย:** {targets} ดอลลาร์")
        bearish_items.append(f"**จุด Stoploss:** เมื่อ{down_scenario['invalidation']}")
        ...
        if down_scenario.get("entry_invalidation") is not None:
            lines += ["**ขั้นตอนเข้าเทรดฝั่งลง (Breakdown-Continuation):** "
                      "สำหรับคนที่อยากตามแรงลง", ""]
            lines += wcb_writers.listing("", [
                f"**ขั้นที่ 1 —** รอให้ราคาปิดวันใต้ {money(down_scenario['trigger'])} "
                "ดอลลาร์ให้จบก่อน อย่าไล่ราคาตอนกำลังหลุด",
                f"**ขั้นที่ 2 —** รอจังหวะราคาเด้งกลับมาทดสอบแนวที่เพิ่งหลุดในช่วง "
                f"{money(down_scenario['entry_low'])}–"
                f"{money(down_scenario['entry_high'])} ดอลลาร์ "
                "(Retest — แนวรับเดิมพลิกเป็นแนวต้าน)",
                "**ขั้นที่ 3 —** หาสัญญาณแรงขายยืนยันใน Timeframe ย่อยก่อน "
                "แล้วจึงพิจารณาเข้า",
                f"**ขั้นที่ 4 —** จุด Stoploss: ราคาปิดวันกลับสูงกว่า "
                f"{money(down_scenario['entry_invalidation'])} ดอลลาร์ — "
                "ถึงตรงนั้นการหลุดถือว่าล้มเหลว ไม่ถัวไม่รอครับ",
            ]) + [""]
    ...
    if calendar and calendar.get("sentences"):
        ...
        lines += [*RULE, heads.head(H2_CALENDAR), "",
                  ...
                  "คือกำหนดการเศรษฐกิจข้างหน้า ไล่รายการที่ใกล้ที่สุดจากปฏิทินจริงของระบบ ดังนี้:", ""]
        groups = wcb_writers.calendar_day_groups(sentences, calendar.get("events") or [])
        if groups is None:
            lines += ["ด่านแรกคือ" + sentences[0]
                      + (" ต่อด้วย " + " ต่อด้วย ".join(sentences[1:]) if sentences[1:] else ""),
                      ""]
        else:
            lines += wcb_writers.nested_listing(groups)
            if lines[-1] != "":
                lines.append("")
        lines += ["บทนี้จงใจไม่เดาย้อนหลังว่าราคาที่ผ่านมาขยับเพราะข่าวใด — "
    ...
    question = "ทั้งกระดานวันนี้ย่อลงเหลือคำถามเดียวครับ — "
    if up and zones:
        question += (
        ...
        question += f"ราคาจะผ่านด่าน {money(up['trigger'])} ดอลลาร์ได้หรือไม่"
    else:
        question += "โครงสร้างจะเลือกทางไหน คำตอบอยู่ที่ราคาปิดเทียบระดับบนภาพ ไม่ใช่การเดา"
    lines += [question, ""]

    summary_items: list[str] = []
    upper_gate = money(up["trigger"]) + " ดอลลาร์" if up else None
    lower_gate = (money(down_scenario["trigger"]) + " ดอลลาร์" if down_scenario
                  else (money(zones[0]["mean"]) + " ดอลลาร์" if zones else None))
    if upper_gate or lower_gate:
        watch = "**ต้องดูอะไร:** ราคาปิดรายวันเทียบ"
        if upper_gate and lower_gate:
            watch += f"สองระดับ — ด่านบน {upper_gate} กับด่านล่าง {lower_gate}"
        else:
            watch += f"ระดับเดียวที่เหลืออยู่คือ {upper_gate or lower_gate}"
        watch += " อย่างอื่นเป็นเสียงรบกวนระหว่างวัน"
        summary_items.append(watch)
        why = "**ทำไมต้องดูราคาปิด:** ราคาระหว่างวันแหย่ทะลุแล้วดีดกลับได้เสมอ (Stop Hunt) "
        why += "แต่ราคาปิดคือคำตัดสินที่โครงสร้างยอมรับจริง — "
        parts = []
        if upper_gate:
            parts.append(f"ปิดเหนือ {upper_gate} = Break of Structure ฝั่งขึ้น")
        if lower_gate:
            parts.append(f"ปิดใต้ {lower_gate} = Demand ถูกเจาะ โครงสร้างเปลี่ยนฝั่ง")
        why += " · ".join(parts)
        summary_items.append(why)
        summary_items.append(
            "**ทำอย่างไร:** ระหว่างที่ยังไม่ปิดทะลุฝั่งใด ให้เดินตามลำดับขั้นในหัวข้อ 3 — "
            "รอราคาเข้าโซน รอสัญญาณยืนยันใน Timeframe ย่อย แล้วจึงพิจารณา "
            "ไม่ไล่ราคากลางกรอบ และวางจุด Stoploss ก่อนเข้าทุกครั้ง")
        ...
        if down_scenario:
            ...
            outcomes.append(outcome)
        outcomes.append("ราคายังไม่ถึงระดับไหนเลย = รอต่อโดยไม่ต้องเดา "
                        "เพราะทั้งหมดเป็นเงื่อนไข ไม่ใช่คำทำนาย")
        then += " · ".join(outcomes)
        summary_items.append(then)
