...
#
# 📏 **ช่วง 380–780 คำ — วัดจากบทจริงทั้งสามสไตล์ก่อนตั้ง ไม่ได้เดา**
...
# โครงของ H/I/J บังคับสามอย่างที่บทรายวันไม่มี — หัวข้ออธิบายศัพท์ (~90 คำ) ·
# ย่อหน้าเชื่อมภาพกับข้อสรุปสองใบ (~110 คำ) · กล่องสถานะหัวบท ⇒ รวมราว 200 คำที่เป็น
# **ภาคบังคับตามข้อเสนอ §57–58** ไม่ใช่ความฟุ่มเฟือย · ตัวเนื้อวิเคราะห์จริงยังอยู่ในช่วง
# เดียวกับบทรายวัน ถ้าตั้งเพดานที่ 560 เท่ากัน = บังคับให้ตัดชั้นคำอธิบายทิ้ง
# ซึ่งเป็นชั้นที่ทำให้มันเป็น "สไตล์บทวิเคราะห์" แทนที่จะเป็น "เครื่องปั๊มสัญญาณ"
#
# ต่ำกว่า 380 แปลว่ามีหัวข้อหายไปทั้งหัว (โครงมีหกหัวข้อ + สองภาพ)
MIN_WORDS, MAX_WORDS = 380, 780
...
H2_GLOSSARY = "อ่านศัพท์ในบทนี้แบบเข้าใจง่าย"
...
def render_article(story: dict, spec) -> str:
    ...
    lines += [*RULE, heads.head(H2_GLOSSARY), ""]
    lines += spec.glossary_lines(story)
    lines += [""] if lines[-1] != "" else []
