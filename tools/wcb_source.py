...
#
# ⛔ **สองช่องนี้ลอกมาจากเอกสารสเปกของหัวหน้าทีละตัวอักษร ห้ามเขียนเอาเองจากความรู้สึก**
# ที่มา: `01-Lead/Input/10-08-2026/` — โภคภัณฑ์ §2 · Forex §1 · คริปโต §1 · หุ้น §1
# แต่ละหมวดมีกฎของตัวเองที่ขัดกับสามัญสำนึก ถ้าไม่เปิดไฟล์อ่านจะแก้ผิดทาง เช่น
# `EUR/USD` ต้องใช้ ticker ในพาดหัว **ห้ามใช้คำว่า "ค่าเงินยูโร"** เพราะคีย์นั้นเป็นของ
# หน้า EUR/THB (Forex กฎข้อ 8) — เหตุผลคือ cannibalization ไม่ใช่เรื่องความสวยงาม
ASSET_SEO_SPEC_SOURCE = "01-Lead/Input/10-08-2026/ (สเปกพาดหัวของหัวหน้า · ล็อก 2026-08-10)"
ASSET_PROFILES = {
    "xauusd": {
        "thai_name": "ทองคำโลก",
        "short_name": "ทอง",
    ...
    "eurusd": {
        ...
        "short_name": "ยูโร",
        # ⛔ **ห้ามใส่คำว่า "ค่าเงินยูโร" ในพาดหัวหน้านี้** (Forex กฎข้อ 8) — คีย์นั้น
        # (60,500/เดือน) เป็นเจตนา "ยูโรต่อบาท" ซึ่งเป็นของหน้า EUR/THB · สองหน้าพูดถึง
        # เงินสกุลเดียวกัน ใส่ปนกันเมื่อไหร่คือแย่งคีย์กันเอง · สเปกจึงสั่งให้หน้านี้ใช้
        # ticker เป็นชื่อ เพื่อแยกตัวเองออกจากหน้า THB ให้ชัดทั้งกับ Google และคนอ่าน
        # (วันนี้เรายังไม่มีหน้า EUR/THB ⇒ ยังไม่ชนจริง แต่ทำตามสเปกไว้ก่อนถูกกว่าแก้ทีหลัง)
        "seo_name": "EUR/USD",
        "seo_tail": "แนวโน้มยูโรต่อดอลลาร์",
        "symbol": "EUR/USD",
    ...
    "gbpusd": {
        ...
        "short_name": "ปอนด์",
        # ต่างจาก EUR/USD ตรงที่หน้านี้ **ถือคีย์ `ค่าเงินปอนด์` (18,100) ได้** เพราะ
        # ระบบไม่มีหน้า GBP/THB ให้แย่ง — สเปกอนุญาตไว้ตรง ๆ (Forex §0 เชิงอรรถ)
        # แต่มีเงื่อนไขพ่วง: หน้านี้ต้องมีหัวข้อ "ปอนด์ต่อบาทวันนี้" ด้วย ไม่งั้นผิดเจตนาคีย์
        "seo_name": "ค่าเงินปอนด์",
        "seo_tail": "แนวโน้ม GBP/USD",
        "symbol": "GBP/USD",
    ...
    "btcusd": {
        # 🔤 **สะกด "บิทคอยน์" (ท) ไม่ใช่ "บิทคอยน์" (ต)** — สเปกคริปโต กฎข้อ 8
        # รูปที่คนไทยค้นจริงคือรูปนี้ (60,500/เดือน) ส่วนรูป ต วัดวอลุ่มแทบไม่ได้
        # ⚠️ **เปลี่ยนทั้งบท ไม่ใช่เฉพาะพาดหัว** (คำตัดสิน CC 2026-08-10) — เดินตาม
        # บรรทัดฐานเดียวกับตอนเปลี่ยนเป็น พ.ศ.: พาดหัวสะกดอย่าง เนื้อบทสะกดอีกอย่าง
        # คือหน้าที่ขัดกันเอง ซึ่งทีมเว็บเคยตีกลับมาแล้ว (D-4.5) ⇒ `thai_name`/
        # `short_name`/`macro` เปลี่ยนตามหมด ไม่ใช่แค่ช่อง seo_*
        "thai_name": "บิทคอยน์",
        "short_name": "บิทคอยน์",
        "seo_name": "บิทคอยน์",
        # หางตามสเปกลงท้ายด้วย "BTC" เปล่า ๆ ไม่ใช่ "BTC/USD" — ลอกมาตามตัวอักษร
        "seo_tail": "แนวโน้มราคาบิทคอยน์ BTC",
        "symbol": "BTC/USD",
        "unit_phrase": "ดอลลาร์",
        "decimals": 2,
        "levels": 0,
        "macro": ("สายส่งจากตัวเลขเหล่านี้ถึงบิทคอยน์เดินผ่านความอยากเสี่ยงของตลาดและสภาพคล่องเป็นหลัก "
    ...
    "nvda": {
        # 🔁 **กลับคำเดิม 2026-08-10 ตามสเปกของหัวหน้า + ผู้ใช้ยืนยันทับอีกครั้ง**
        #
        # เดิมที่นี่เขียนว่า "ห้ามเรียกว่าหุ้นเด็ดขาด" เพราะสินค้าเราเป็น CFD ไม่ใช่หุ้น
        # บนกระดาน · สเปกหุ้น กฎข้อ 7–9 สั่งตรงข้าม: ต้องมีคำว่า `หุ้น` นำหน้า และใช้ชื่อ
        # บริษัทเป็นอังกฤษ (`หุ้น nvidia` 22,200 · `วิเคราะห์หุ้น nvda` 170 — ต่างกัน 37 เท่า)
        # ผู้ใช้รับทราบข้อขัดแย้งนี้แล้วและสั่งให้ทำตามสเปก
        #
        # ⚠️ **สิ่งที่ห้ามหายไปพร้อมกับการกลับคำ:** พาดหัวเรียก "หุ้น" ได้ แต่ **บทต้องบอก
        # ชนิดเครื่องมือจริงเสมอ** — ข้อความนั้นอยู่ใน `unit_phrase` ข้างล่าง และถูกล็อกด้วย
        # เทส `test_headline_format.หุ้นสหรัฐต้องบอกชนิดเครื่องมือ` ⇒ ใครลบทิ้งบทจะตกทันที
        # เหตุผลที่ต้องมีด่าน: เรียกผิดชนิดสินค้าเป็นเรื่อง YMYL ไม่ใช่เรื่อง SEO
        # ถ้าจะถอดข้อความนี้ ต้องถามผู้ใช้ก่อนเสมอ ห้ามถอดเพราะเห็นว่าพาดหัวพูดแล้ว
        # 📐 **ขอบเขตของกฎข้อ 9 (ชื่อบริษัทเป็นอังกฤษ) = พาดหัวเท่านั้น ไม่ใช่เนื้อบท**
        # เหตุผลที่สเปกให้ไว้คือวอลุ่มการค้นหา ซึ่งเป็นเรื่องของ Title/H1 · เนื้อบทเป็น
        # ร้อยแก้วไทยที่เอาชื่อไปต่อกับคำไทยตรง ๆ 13 จุด (`{short_name}อยู่ที่ ...` ·
        # `ราคา{short_name}แบบเรียลไทม์`) ⇒ ใส่อักษรละตินลงไปจะได้ "NVIDIAอยู่ที่" ติดกัน
        # ⇒ ช่องพาดหัวใช้ NVIDIA ตามสเปก · ช่องเนื้อบทคงคำอ่านไทยไว้ให้ประโยคเดินได้
        # (ทองทำแบบเดียวกันอยู่แล้ว: พาดหัว "ทองคำ" · เนื้อบท "ทองคำโลก")
        "thai_name": "เอ็นวิเดีย",
        "short_name": "เอ็นวิเดีย",
        "seo_name": "หุ้น NVIDIA",
        "seo_tail": "แนวโน้มหุ้น NVDA",
        "symbol": "NVDA",
    ...
    # เพิ่ม 2026-08-10 ตามคำสั่งผู้ใช้ · โซลานาเป็นคริปโทเช่นเดียวกับบิทคอยน์แต่ผันผวนกว่า
    # (beta สูงกว่า) — เขียนแยกจากย่อหน้าบิทคอยน์เพื่อไม่ให้อ่านเหมือนเป็นสินทรัพย์เดียวกัน
    "solusd": {
        "thai_name": "โซลานา",
        "short_name": "โซลานา",
        # สเปกคริปโตจัด SOL อยู่กลุ่ม "ไม่มีข้อมูลวอลุ่มภาษาไทย" (รวมกับ XRP/ADA/BNB/USDT)
        # ซึ่งมีแม่แบบของตัวเองคือใช้ ticker ล้วน — ต่างจาก BTC/ETH/DOGE/SHIB ที่สเปกสั่ง
        # ให้ใช้คำทับศัพท์ไทย เพราะสี่ตัวนั้นวัดวอลุ่มคำไทยได้จริง
        # `thai_name` คงเป็น "โซลานา" ไว้ตามเดิม — พาดหัวใช้ ticker เนื้อบทเรียกชื่อไทย
        # ไม่ใช่ความขัดแย้งแบบ บิท/บิต (คนละคำสะกดของคำเดียวกัน) แต่เป็นชื่อเรียกสองแบบ
        # ที่คนอ่านโยงกันได้เอง เหมือน EUR/USD กับ "ยูโร"
        "seo_name": "SOL",
        "seo_tail": "แนวโน้มราคา SOL",
        "symbol": "SOL/USD",
        "unit_phrase": "ดอลลาร์ต่อเหรียญ",
        "decimals": 2,
        "levels": 1,
        "macro": ("สายส่งจากตัวเลขเหล่านี้ถึงโซลานาเดินผ่านความอยากเสี่ยงของตลาดเช่นเดียวกับบิทคอยน์ "
                  "ข้อมูลที่ทำให้ตลาดคาดว่าดอกเบี้ยจะผ่อนลงมักหนุนสินทรัพย์เสี่ยงทั้งกลุ่มพร้อมกัน "
                  "แต่โซลานาเป็นเหรียญที่มีมูลค่าตลาดเล็กกว่าบิทคอยน์มาก ราคาจึงมักแกว่งแรงกว่า "
                  "ทั้งขาขึ้นและขาลงเมื่อเทียบกับการขยับของบิทคอยน์ในรอบข่าวเดียวกัน"),
    },
}
