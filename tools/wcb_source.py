...
ASSET_TAGS = {
    ...
    "usdthb": "usdthb",
    "solusd": "sol",
    # ⚠️ tag ยึด `wti` สั้นตามธรรมเนียม btc/sol — วัดจริง 08-11: `wti` กับ `wtiusd`
    # ตอบ 200 ทั้งคู่ คืน symbol "WTI/USD" ชุดเดียวกันเป๊ะ (790 แท่ง ช่วงเดียวกัน
    # ราคาปิดแท่งท้ายตรงกัน) — ไม่ใช่กับดักสองชุดข้อมูลแบบ btc/btcusd
    "wtiusd": "wti",
}
...
ASSET_PROFILES = {
    ...
    "usdjpy": {
        "thai_name": "ดอลลาร์สหรัฐเทียบเยนญี่ปุ่น",
        "short_name": "ดอลลาร์-เยน",
        "seo_name": "USD/JPY",
        "seo_tail": "แนวโน้มดอลลาร์ต่อเยน",
        "symbol": "USD/JPY",
        "unit_phrase": "เยนต่อดอลลาร์",
        "unit_short": "เยน",
        "decimals": 3,
        "levels": 4,
        "macro": ("คู่เงินนี้ต้องอ่านนโยบายการเงินสองฝั่งควบคู่กัน "
                  "ข้อมูลสหรัฐที่แข็งกว่าคาดมักหนุนดอลลาร์และดัน USD/JPY ขึ้น "
                  "ขณะที่ข้อมูลญี่ปุ่นและท่าทีธนาคารกลางญี่ปุ่นมีผลต่อฝั่งเยนโดยตรง "
                  "สิ่งที่ขยับคู่เงินจริงคือส่วนต่างดอกเบี้ยและความคาดหวังระหว่างเฟดกับธนาคารกลางญี่ปุ่น "
                  "จึงห้ามสรุปทิศจากข่าวของฝั่งใดฝั่งหนึ่งโดยไม่ดูหลักฐานของอีกฝั่ง"),
    },
    "audusd": {
        "thai_name": "ดอลลาร์ออสเตรเลียเทียบดอลลาร์สหรัฐ",
        "short_name": "ออสซี่",
        "seo_name": "AUD/USD",
        "seo_tail": "แนวโน้มดอลลาร์ออสเตรเลียต่อดอลลาร์",
        "symbol": "AUD/USD",
        "unit_phrase": "ดอลลาร์สหรัฐต่อดอลลาร์ออสเตรเลีย",
        "unit_short": "ดอลลาร์สหรัฐ",
        "decimals": 5,
        "levels": 4,
        "macro": ("คู่เงินนี้ต้องอ่านส่วนต่างระหว่างสหรัฐกับออสเตรเลียควบคู่กัน "
                  "ข้อมูลสหรัฐและท่าทีธนาคารกลางสหรัฐมีผลต่อฝั่งดอลลาร์ "
                  "ขณะที่ข้อมูลออสเตรเลียและท่าทีธนาคารกลางออสเตรเลียมีผลต่อฝั่งดอลลาร์ออสเตรเลีย "
                  "จึงห้ามสรุปทิศจากข่าวของฝั่งใดฝั่งหนึ่งโดยไม่ดูหลักฐานของอีกฝั่ง"),
    },
    "usdcad": {
        "thai_name": "ดอลลาร์สหรัฐเทียบดอลลาร์แคนาดา",
        "short_name": "ดอลลาร์-แคนาดา",
        "seo_name": "USD/CAD",
        "seo_tail": "แนวโน้มดอลลาร์สหรัฐต่อดอลลาร์แคนาดา",
        "symbol": "USD/CAD",
        ...
        "macro": ("คู่เงินนี้ต้องอ่านส่วนต่างระหว่างสหรัฐกับแคนาดาควบคู่กัน "
                  ...
                  "ขณะที่ข้อมูลแคนาดา ท่าทีธนาคารกลางแคนาดา และความคาดหวังต่อเศรษฐกิจแคนาดามีผลต่อฝั่งดอลลาร์แคนาดา "
                  "ราคาพลังงานอาจเป็นบริบทประกอบ แต่ไม่ใช้แทนหลักฐานราคาและแท่งปิด"),
    },
    "btcusd": {
    ...
    "nvda": {
        ...
        # ⚠️ รูปสั้นตัดคำว่า "ซื้อขายผ่านสัญญาส่วนต่าง" ออกโดยเจตนา เพราะใช้ต่อท้ายเลข
        # ในรายการทีละบรรทัด — ข้อความ CFD ยังอยู่ครบในประโยคเปิดบท (`_opening` ใช้
        # `unit_phrase` เต็ม) ซึ่งเป็นที่ที่ด่าน YMYL ต้องการ · ห้ามเอารูปสั้นไปแทน
        # ประโยคเปิด ไม่งั้นชนิดเครื่องมือจะหายจากบททั้งใบ
        "unit_short": "ดอลลาร์",
        "decimals": 2,
        "levels": 1,
    ...
    "usdthb": {
        ...
        # 🐞 ตัวที่เปิดโปงบั๊ก 08-11: จุดที่เขียน "ดอลลาร์" ตายตัวทำให้คู่นี้บอกหน่วยผิด
        # (ราคากับส่วนต่างรายวันของ USD/THB เป็น**บาท**) ⇒ ทุกจุดต้องอ่านช่องนี้
        "unit_short": "บาท",
        "decimals": 5,
        "levels": 4,
    ...
    # เพิ่ม 2026-08-11 ตามคำสั่งผู้ใช้ · สเปกหัวหน้า: โภคภัณฑ์ §2 (🛢️ WTI)
    # ตัวแรกในทะเบียนที่ **Title กับ H1 ใช้ชื่อคนละตัว** ตามสเปกทีละตัวอักษร:
    #   Title: "วิเคราะห์น้ำมันดิบวันนี้ … — แนวโน้มราคาน้ำมัน WTI"   (ชื่อ "น้ำมันดิบ")
    #   H1:    "วิเคราะห์น้ำมันดิบ WTI วันนี้ … — WTI ยืน {ราคา} …"  (ชื่อ "น้ำมันดิบ WTI")
    # จึงมีช่อง `seo_h1_name` เพิ่ม — สินทรัพย์อื่นไม่มีช่องนี้ = สองที่ใช้ชื่อเดียวกันตามเดิม
    "wtiusd": {
        "thai_name": "น้ำมันดิบ",
        "short_name": "น้ำมันดิบ",
        "seo_name": "น้ำมันดิบ",
        "seo_h1_name": "น้ำมันดิบ WTI",
        "seo_tail": "แนวโน้มราคาน้ำมัน WTI",
        "symbol": "WTI/USD",
        "unit_phrase": "ดอลลาร์ต่อบาร์เรล",
        "unit_short": "ดอลลาร์",
        "decimals": 2,
        "levels": 1,
        "macro": ("สายส่งจากตัวเลขเหล่านี้ถึงราคาน้ำมันดิบเดินสองทางที่หักล้างกันได้ในวันเดียว "
                  "ทางแรกคือดอลลาร์ เพราะน้ำมันตั้งราคาเป็นดอลลาร์ "
                  "ข้อมูลสหรัฐที่แข็งกว่าคาดหนุนดอลลาร์และทำให้น้ำมันแพงขึ้นสำหรับผู้ซื้อสกุลอื่น ซึ่งกดราคาได้ "
                  "ทางที่สองแรงกว่าคือความคาดหวังต่อเศรษฐกิจจริง "
                  "ตัวเลขที่แข็งแปลว่าอุปสงค์พลังงานยังโต ซึ่งหนุนราคาน้ำมันแม้ดอลลาร์จะแข็งขึ้นพร้อมกัน "
                  "ส่วนตัวขับหลักของตลาดนี้ — สต็อกน้ำมันดิบสหรัฐ มติกำลังการผลิตของกลุ่มผู้ผลิต "
                  "และเหตุภูมิรัฐศาสตร์ — ไม่ได้อยู่ในชุดข้อมูลนี้ "
                  "บทจึงอ่านได้เฉพาะแรงขับจากปฏิทินมหภาคสหรัฐเท่านั้น"),
    },
}
